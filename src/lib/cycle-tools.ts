...
export function diffInDays(startIso: string, endIso: string): number {
  return Math.round((toUtcDate(endIso).getTime() - toUtcDate(startIso).getTime()) / DAY_MS)
}
...
export function normalizeDateInput(
  ...
  if (lower === "yesterday") {
    return { isoDate: addDaysToIsoDate(getCurrentIsoDate(resolvedTimeZone, reference), -1) }
  }
  ...
  if (monthMatch?.groups) {
    ...
    const year = Number(monthMatch.groups.year ?? getDatePartsInTimeZone(reference, resolvedTimeZone).year)
...
export function normalizeCycleNotes(notes: unknown): CycleNotes {
  ...
  for (const [dateKey, value] of Object.entries(notes as Record<string, unknown>)) {
    ...
    if (value && typeof value === "object") {
      ...
      if (typeof record.symptom === "string" && record.symptom.trim().length > 0) {
...
async function refreshPredictionParam(
  ...
  const blended = blendWithPrior(smoothed, variance, observations.length, metricName)

  const existing = await db
    .select()
    .from(predictionParams)
    .where(eq(predictionParams.userId, userId))
    .where(eq(predictionParams.paramName, paramName))
    .limit(1)
...
async function refreshCycleAnalytics(userId: string): Promise<AnalyticsResult> {
  ...
  for (let index = 0; index < normalizedRows.length; index += 1) {
    ...
    const nextCycleLength = previous ? diffInDays(previous.mStart, current.mStart) : null
    const nextPeriodLength = current.mEnd ? diffInDays(current.mStart, current.mEnd) : null
    ...
    const nextLutealLength =
      current.ovulationDate && next ? diffInDays(current.ovulationDate, next.mStart) : null
    ...
    if (typeof nextPeriodLength === "number") periodLengths.push(nextPeriodLength)
    if (typeof nextFollicularLength === "number") follicularLengths.push(nextFollicularLength)
    if (typeof nextLutealLength === "number") lutealLengths.push(nextLutealLength)
    ...
    if (current.cycleLength !== nextCycleLength) changes.cycleLength = nextCycleLength
    if (current.periodLength !== nextPeriodLength) changes.periodLength = nextPeriodLength
    if (current.follicularLength !== nextFollicularLength) changes.follicularLength = nextFollicularLength
    if (current.lutealLength !== nextLutealLength) changes.lutealLength = nextLutealLength
...
async function getPredictionParamMap(userId: string) {
  ...
  return rows.reduce<Partial<Record<PredictionParamName, PredictionParamRow>>>((acc, row) => {
...
function buildPredictionPayload(analytics: AnalyticsResult) {
  const cyclePrediction = predictNextCycle(analytics.cycleLengths, "cycleLength")
  const periodPrediction = predictNextCycle(analytics.periodLengths, "periodLength")
  const lutealPrediction = predictNextCycle(analytics.lutealLengths, "lutealLength")
  ...
  const nextPeriodEnd = nextPeriodStart
    ? addDaysToIsoDate(nextPeriodStart, Math.max(0, Math.round(periodPrediction.predicted)))
    : null
  const nextOvulationDate = nextPeriodStart
    ? addDaysToIsoDate(nextPeriodStart, -Math.max(1, Math.round(lutealPrediction.predicted || 14)))
    : null
...
function buildAveragesFromParams(
  ...
  return {
    cycleLength: paramMap.cycle_length?.smoothedValue ?? predictNextCycle(analytics.cycleLengths, "cycleLength").predicted,
    periodLength: paramMap.period_length?.smoothedValue ?? predictNextCycle(analytics.periodLengths, "periodLength").predicted,
    follicularLength:
      paramMap.follicular?.smoothedValue ?? predictNextCycle(analytics.follicularLengths, "follicularLength").predicted,
    lutealLength: paramMap.luteal?.smoothedValue ?? predictNextCycle(analytics.lutealLengths, "lutealLength").predicted,
  }
}
...
export async function logPeriodStartEntry(args: {
  ...
  if (!normalized.isoDate) {
    return {
      ...
      question: "I need a clear start date before I can log that period. What date should I use?",
      reason: normalized.reason,
    }
  }

  const analytics = await refreshCycleAnalytics(args.userId)
  const existing = analytics.cycles.find((row) => row.mStart === normalized.isoDate)
  const noteText = typeof args.notes === "string" ? args.notes.trim() : ""

  if (existing) {
    const mergedNotes = noteText.length > 0 ? mergeNoteEntries(existing.notes, normalized.isoDate, [noteText]) : existing.notes
    ...
    const cycle = refreshed.cycles.find((row) => row.id === existing.id) ?? existing

    return {
      responseMode: "plain" as const,
      kind: "confirmation" as const,
      message: noteText.length > 0
  ...
  const notes = noteText.length > 0 ? mergeNoteEntries({}, normalized.isoDate, [noteText]) : {}
...
export async function logPeriodEndEntry(args: {
  ...
  if (!normalized.isoDate) {
    return {
      ...
      kind: "clarification" as const,
      needsClarification: true,
      question: "I need a clear end date before I can log that period. What date should I use?",
      reason: normalized.reason,
    }
  }
  ...
  const target = analytics.cycles
    .slice()
    .reverse()
    .find((row) => row.mStart <= normalized.isoDate && (!row.mEnd || row.mEnd >= normalized.isoDate))
  ...
  if (!target) {
    return {
      ...
      kind: "clarification" as const,
      needsClarification: true,
      question: "I could not find an open period to close. What start date should I attach this end date to?",
      date: normalized.isoDate,
    }
  }

  const noteText = typeof args.notes === "string" ? args.notes.trim() : ""
  const mergedNotes = noteText.length > 0 ? mergeNoteEntries(target.notes, normalized.isoDate, [noteText]) : target.notes
...
export async function logOvulationEntry(args: {
  ...
  if (!normalized.isoDate) {
    return {
      ...
      kind: "clarification" as const,
      needsClarification: true,
      question: "I need a clear ovulation date before I can log it. What date should I use?",
      reason: normalized.reason,
    }
  }
  ...
  if (!target) {
    return {
      ...
      kind: "clarification" as const,
      needsClarification: true,
      question: "I could not find a cycle to attach that ovulation date to. What period start date should I use?",
      date: normalized.isoDate,
    }
  }
...
export async function addCycleNoteEntry(args: {
  ...
  const resolvedDate = normalizeDateInput(args.date ?? getCurrentIsoDate(args.timeZone), args.timeZone)
  if (!resolvedDate.isoDate) {
    return {
      responseMode: "plain" as const,
      kind: "clarification" as const,
      needsClarification: true,
      question: "I could not resolve the note date. What date should I attach it to?",
  ...
  const target = analytics.cycles
    .slice()
    .reverse()
    .find((row) => row.mStart <= resolvedDate.isoDate! && (!row.mEnd || row.mEnd >= resolvedDate.isoDate!))
  ...
  if (!target) {
    return {
      ...
      kind: "clarification" as const,
      needsClarification: true,
      question: "I need at least one logged cycle to attach that note. Would you like to log a period start first?",
    }
  }
  ...
  const mergedNotes = mergeNoteEntries(target.notes, resolvedDate.isoDate, entries)
...
export async function fetchRecentCyclesEntry(args: {
  ...
  const analytics = await refreshCycleAnalytics(args.userId)
  const recentCycles = analytics.cycles.slice(-args.limit).reverse().map(summarizeCycle)
...
export async function getCycleInsightsEntry(args: {
  ...
  const lastCycle = analytics.cycles.at(-1) ? summarizeCycle(analytics.cycles.at(-1) as CycleRow) : null
  ...
  if (!hasCycles) {
    return {
      responseMode: "plain" as const,
      kind: args.mode,
      message: "I do not have enough cycle data yet. Log a period start first and I can build predictions and stats.",
      recentCycles,
      lastCycle,
      averages,
      predictions,

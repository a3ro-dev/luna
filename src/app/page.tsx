...
export default function Home() {
  ...
  useGSAP(() => {
    ...
    mm.add("(prefers-reduced-motion: no-preference)", () => {
      ...
      // 1. HERO SECTION (Jakub-style smooth blur entrance)
      ...
      // Pin hero text & scale background gently
      ...
      // 2. TYPOGRAPHIC FEATURE REVEAL (Impeccable: No identical cards)
      gsap.utils.toArray<HTMLElement>('.feature-row').forEach((row, i) => {
        gsap.from(row, {
          ...
          opacity: 0,
          x: i % 2 === 0 ? -30 : 30, // Alternate entry directions
          filter: "blur(4px)",
          duration: 1,
          ease: "power2.out"
        });
      ...
      narrativeTl.to(narrativeText1Ref.current, { opacity: 0, y: -40, duration: 1 })
                 .fromTo(narrativeText2Ref.current, { opacity: 0, y: 40 }, { opacity: 1, y: 0, duration: 1 }, "<0.5")
                 .to(narrativeText2Ref.current, { opacity: 0, y: -40, duration: 1 })
                 .fromTo(narrativeText3Ref.current, { opacity: 0, y: 40 }, { opacity: 1, y: 0, duration: 1 }, "<0.5");
      ...
      if (canvas) {
        ...
        const render = () => {
          if (images[frames.frame] && context) {
            ...
            // Scale up by 1.25 and shift Y up by 80 to crop watermark
        ...
        gsap.to(frames, {
          ...
          scrollTrigger: {
            ...
            scrub: 0.5, // 0.5 sec scrub lag for smoothness
          },
          onUpdate: render,
        });
      }
  ...
  return (
    <div ref={container} className="bg-[#FCFBFB] font-sans selection:bg-[#F7C4C8] selection:text-white overflow-x-hidden">
      ...
      <section ref={heroRef} className="relative h-screen w-full flex flex-col items-center justify-center z-10">
        <div 
          ref={heroBgRef} 
          className="absolute inset-0 w-full h-full bg-gradient-to-br from-[#FCFBFB] via-[#F7C4C8]/30 to-[#FCFBFB] -z-10 will-change-transform"
        />
        
        <div ref={heroTextRef} className="text-center px-6 will-change-transform max-w-4xl">
          <h1 className="font-serif text-7xl md:text-[9rem] text-[#5A4A4D] mb-6 tracking-tight leading-none drop-shadow-sm">
            Luna
          </h1>
          <p className="text-xl md:text-3xl text-[#7A6A6D] opacity-90 max-w-2xl mx-auto font-medium leading-relaxed">
            Your beautiful, intelligent cycle tracker. Designed for peace of mind.
          </p>
        </div>
      </section>

      {/* 2. TYPOGRAPHIC FEATURE REVEAL (Impeccable style: No identical cards) */}
      <section ref={featuresContainerRef} className="py-40 px-6 md:px-24 max-w-6xl mx-auto z-10 relative">
        <div className="mb-32 max-w-2xl">
          <h2 className="font-serif text-5xl text-[#5A4A4D] mb-6">Intelligently soft.</h2>
          <p className="text-2xl text-[#7A6A6D] opacity-80 leading-relaxed">
        ...
        <div className="flex flex-col gap-24 md:gap-32">
          {[
            ...
            { num: "03", title: "Luna AI", desc: "A supportive, knowledgeable companion available 24/7. Built on deeply personalized context to understand your history.", icon: <Sparkles className="text-[#F4A6A6] w-8 h-8" /> }
          ].map((feature, i) => (
            <div key={i} className="feature-row will-change-transform flex flex-col md:flex-row items-start gap-8 md:gap-16">
              <span className="font-serif text-[#F7C4C8] text-6xl md:text-8xl opacity-50 tracking-tighter leading-none shrink-0">
                {feature.num}
              </span>
              <div className="max-w-xl">
                <div className="bg-[#F7C4C8]/20 w-16 h-16 flex items-center justify-center rounded-2xl mb-8">
                  {feature.icon}
                </div>
                <h3 className="font-serif text-4xl text-[#5A4A4D] mb-6">{feature.title}</h3>
                <p className="text-[#7A6A6D] text-xl opacity-90 leading-relaxed">{feature.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section ref={narrativeContainerRef} className="h-screen w-full relative bg-white/50 flex items-center overflow-hidden z-10 border-y border-[#F7C4C8]/10">
        ...
        <div className="w-full md:w-1/2 h-full flex items-center justify-center p-8 md:p-12 absolute md:relative opacity-20 md:opacity-100 -z-10 md:z-auto">
          <div className="w-full max-w-lg aspect-[3/4] bg-[#FCFBFB] rounded-[2.5rem] shadow-2xl shadow-[#F7C4C8]/20 border-8 border-white relative overflow-hidden flex items-center justify-center">
             {/* The canvas scales to cover, creating an immersive window */}
        ...
        <div className="w-full md:w-1/2 h-full relative flex flex-col justify-center px-8 md:px-24 pointer-events-none">
          <div ref={narrativeText1Ref} className="absolute w-full max-w-lg pr-12 will-change-transform">
            ...
            <h2 className="font-serif text-5xl md:text-6xl text-[#5A4A4D] mb-6 leading-tight">Understand your phases.</h2>
          ...
          <div ref={narrativeText2Ref} className="absolute w-full max-w-lg pr-12 opacity-0 will-change-transform">
            ...
            <h2 className="font-serif text-5xl md:text-6xl text-[#5A4A4D] mb-6 leading-tight">Listen to your body.</h2>
          ...
          <div ref={narrativeText3Ref} className="absolute w-full max-w-lg pr-12 opacity-0 will-change-transform">
            ...
            <h2 className="font-serif text-5xl md:text-6xl text-[#5A4A4D] mb-6 leading-tight">Grok the pattern.</h2>
            <p className="text-2xl text-[#7A6A6D] opacity-90 leading-relaxed font-medium">Powered by intelligent context, Luna remembers your history and provides deeply personalized clarity.</p>
          </div>
        </div>
      </section>

      {/* 4. PARALLAX MEDIA (Impeccable: Elegant typography + Image) */}
      ...
      <section className="py-40 px-6 text-center z-10 relative bg-[#FCFBFB]">
        ...
        <Link 
          ...
          className="inline-flex h-16 items-center justify-center rounded-full bg-[#F7C4C8] hover:bg-[#F4A6A6] px-14 font-bold text-white shadow-lg shadow-[#F7C4C8]/30 transition-colors will-change-transform text-lg"
        >
          Start Tracking
        </Link>
      </section>

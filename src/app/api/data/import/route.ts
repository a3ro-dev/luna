...
function parsePeriodCalendar(text: string): ParsedCycle[] {
  ...
  const events: { date: string; type: "start" | "end" | "other"; raw: string }[] = []
  ...
  for (let i = 0; i < starts.length; i++) {
    ...
    const end = ends.find((e) => e.date > start.date && (!starts[i + 1] || e.date <= starts[i + 1].date))

    const periodLength = end ? daysBetween(start.date, end.date) : null
    const cycleLength = starts[i + 1] ? daysBetween(start.date, starts[i + 1].date) : null

    // Collect notes (weight, symptoms, etc.) for this period window
    const noteEvents = events.filter(
      (e) => e.type === "other" && e.date >= start.date && (!end || e.date <= end.date)
    )
...
function parseClue(text: string): ParsedCycle[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!dateStr || !periodVal || periodVal === "" || periodVal === "spotting") continue
...
function parseFlo(text: string): ParsedCycle[] {
  ...
  if (text.includes(",") && text.split(/\r?\n/)[0].toLowerCase().includes("date")) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (flowIdx >= 0 && (!flowVal || flowVal === "none" || flowVal === "")) continue
...
function parseAppleHealth(text: string): ParsedCycle[] {
  const periodDates: string[] = []
  const regex = /<Record[^>]*type="HKCategoryTypeIdentifierMenstrualFlow"[^>]*startDate="([^"]+)"[^>]*\/>/g
  ...
  const regex2 = /<Record[^>]*startDate="([^"]+)"[^>]*type="HKCategoryTypeIdentifierMenstrualFlow"[^>]*\/>/g
...
function groupConsecutiveDates(dates: string[]): ParsedCycle[] {
  ...
  for (let i = 1; i < dates.length; i++) {
    ...
    if (gap > 2) {
      ...
      const periodLength = daysBetween(start, prev)
      cycles.push({
        mStart: start,
...
function detectFormat(text: string): string {
  ...
  // Luna JSON
  if (trimmed.startsWith("{") && trimmed.includes('"format"') && trimmed.includes('"luna"')) {
    return "luna"
  }
  // Generic JSON array
  ...
  // Apple Health XML
  if (trimmed.includes("HKCategoryTypeIdentifierMenstrualFlow") || trimmed.includes("<HealthData")) {
    return "apple_health"
  }
  ...
  if (trimmed.split(/\r?\n/)[0].toLowerCase().includes("period") && trimmed.includes(",")) {
    return "clue"
  }
  // Flo CSV (has "flow" header)
  if (trimmed.split(/\r?\n/)[0].toLowerCase().includes("flow") && trimmed.includes(",")) {
    return "flo"
  }
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { data, format: explicitFormat } = body as { data: string; format?: string }
    ...
    if (parsed.length === 0) {
      return NextResponse.json({ error: "No cycle data found in the import." }, { status: 400 })
    }

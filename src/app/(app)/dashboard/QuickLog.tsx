...
const primary =
  "inline-flex min-h-12 items-center justify-center rounded-full bg-[var(--tier-ink)] px-7 text-[11px] font-semibold uppercase tracking-widest text-[var(--tier-surface)] shadow-[0_12px_24px_rgba(109,90,96,0.18)] transition-[background-color,scale] duration-150 hover:bg-[var(--tier-muted)] active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50 motion-reduce:active:scale-100 cursor-pointer";
const ghost =
...
// Buttons and date picker swap in place with a short rise.
const swap = {
...
export default function QuickLog({ today, openPeriod }: { today: string; openPeriod: OpenPeriod | null }) {
  ...
  return (
    <section
      ...
      aria-busy={busy}
      className="relative rounded-3xl border border-[var(--tier-line)] bg-[var(--tier-surface)] p-5 shadow-[0_20px_40px_rgba(255,181,192,0.06)] sm:p-7"
    >
      ...
      {/* popLayout: the outgoing block leaves the flow at once, so the card never holds both. */}
      <AnimatePresence initial={false} mode="popLayout">
        {picking ? (
          ...
          <motion.div key="now" {...swap} className="mt-5 flex flex-wrap items-center gap-x-4 gap-y-2">
            <button
              type="button"
              disabled={busy}

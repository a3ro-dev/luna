...
import { components } from "@openuidev/react-ui" // Assuming default components
      ...
      <main className="flex-1 overflow-y-auto p-8 space-y-6">
        ...
        {messages.map(m => (
          <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            >
              {m.role === 'assistant' ? (
                // Use OpenUI Lang Renderer for AI responses to dynamically generate UI
                <Renderer content={m.content} components={components} />
              ) : (
      ...
      <div className="p-6 bg-white border-t border-[#F7C4C8]/20">
        <form onSubmit={onSubmit} className="max-w-4xl mx-auto">
            ...
            <div className="flex gap-2 mb-3 overflow-x-auto pb-2">
              {images.map((file, i) => (
                <div key={i} className="relative w-16 h-16 rounded-lg overflow-hidden border border-[#F7C4C8]/40 shrink-0">
                  ...
                  <button type="button" onClick={() => removeImage(i)} className="absolute top-1 right-1 bg-black/40 text-white rounded-full w-4 h-4 flex items-center justify-center text-[10px]">×</button>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="relative flex items-center bg-[#FCFBFB] border border-[#F7C4C8]/30 rounded-full shadow-sm focus-within:ring-2 focus-within:ring-[#F7C4C8]/50 focus-within:border-[#F7C4C8] transition-all">
            <button type="button" onClick={() => fileInputRef.current?.click()} disabled={images.length >= 4 || totalImagesInContext >= 10} className="p-3 ml-2 text-[#7A6A6D] hover:text-[#F4A6A6] transition-colors disabled:opacity-50">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/></svg>
            </button>
            <input type="file" multiple accept="image/*" className="hidden" ref={fileInputRef} onChange={handleFileChange} />
            <input className="flex-1 bg-transparent border-none py-4 px-2 text-[#5A4A4D] placeholder:text-[#7A6A6D]/50 focus:outline-none focus:ring-0" value={input} placeholder="How are you feeling today?" onChange={handleInputChange} />
            <button type="submit" disabled={isLoading || (!input.trim() && images.length === 0)} className="mr-2 bg-[#F7C4C8] hover:bg-[#F4A6A6] text-white p-2.5 rounded-full transition-colors disabled:opacity-50 disabled:hover:bg-[#F7C4C8]">
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m22 2-7 20-4-9-9-4Z"/><path d="M22 2 11 13"/></svg>
            </button>
          </div>
          <div className="flex justify-between items-center mt-2 px-4">
            <span className="text-[11px] text-[#7A6A6D]/60 flex items-center">{totalImagesInContext}/10 images used in this session</span>
            <span className="text-[11px] text-[#7A6A6D]/40">Luna can make mistakes. Consider verifying important information.</span>
          </div>
        </form>
      </div>
    </div>

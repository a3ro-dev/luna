...
Two questions at logging time reduce these artifacts at the source. A new start within 15 days of an existing one asks whether it is the same period or a separate one before saving. When a saved start closes a gap that the rule above sets aside as too long (and the gap is at least 1.5 times the forecast's typical length), chat and the dashboard ask once whether a period in between went unlogged and suggest a date: the previous start plus the gap divided by the likely number of cycles, kept at least 15 days from both neighbours. The dashboard's quick log asks the same-period question through the quick-log route, which returns a confirmation request (HTTP 409) until the user confirms, and offers the missed-period date picker bounded to those dates. Nothing is inferred or written without the user's answer. A pre-registered simulator study (R4-1) found that if users confirmed half of their missed periods, the 80% interval score would fall 17% and user-macro error 0.68 days in the base scenario; confirming half of the duplicate logs lowered the interval score 7%. Every scenario kept the sign. How often real users answer is unknown. [Logging nudges](../src/lib/prediction/log-nudges.ts) · [Tests](../src/lib/prediction/__tests__/log-nudges.test.ts) · [R4-1 protocol](../autoresearch/experiments/R4-1-artifact-voi/protocol.md)

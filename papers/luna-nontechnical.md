...
You can log a period start or end from the dashboard in one tap ("it started today", "it ended today", or another day), without opening chat. These buttons use the same checks as chat. Chat and dashboard use the same forecast service. Chat confirmations are tied to completed database writes, and stored chat messages retain their structured parts when a stream ends. This reduces the chance of chat and the calendar telling different stories about the same records.

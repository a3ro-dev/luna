...
| Chat storage and streaming | [store.ts](../src/lib/chat/store.ts), [streams.ts](../src/lib/chat/streams.ts), [chat route](../src/app/api/chat/route.ts), [reply-id.test.ts](../src/lib/chat/__tests__/reply-id.test.ts) | One-round-trip chat reads and writes keyed by message id, replies that run to completion and save after a disconnect, resumable streams and Stop through Redis. |
| Logging nudges | [log-nudges.ts](../src/lib/prediction/log-nudges.ts), [log-nudges.test.ts](../src/lib/prediction/__tests__/log-nudges.test.ts), [R4-1 protocol](../autoresearch/experiments/R4-1-artifact-voi/protocol.md) | Same-period check within 15 days and the missed-log suggestion after a set-aside long gap. |
...
| Dashboard summaries | [PremiumDashboard.tsx](<../src/app/(app)/dashboard/PremiumDashboard.tsx>), [premium/](<../src/app/(app)/dashboard/premium/CirclingBack.tsx>), [PremiumPlusDashboard.tsx](<../src/app/(app)/dashboard/PremiumPlusDashboard.tsx>), [premium-plus/](<../src/app/(app)/dashboard/premium-plus/Pages.tsx>), [insights.ts](../src/lib/dashboard/insights.ts) | Circling-back notes, cycle-length sparkline and the Premium+ journal, all derived from the same records every plan has. |
| Light and dark | [mode.ts](../src/lib/theme/mode.ts), [no-flash.ts](../src/lib/theme/no-flash.ts), [contrast.test.ts](../src/lib/theme/__tests__/contrast.test.ts), [globals.css](../src/app/globals.css) | Appearance choice (stored in the browser), pre-paint theme script, per-plan night palettes and their contrast checks. |

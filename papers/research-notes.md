...
Releases 0.11.2--0.13.0 (26 September 2026) fixed four defects found in a code audit. None changes the forecast model. (1) Chat history stored a base64 copy of every uploaded photo inside the message record, so the published 7-day photo deletion did not cover it; history now stores a reference and the daily job strips older copies. (2) Onboarding never saved the perimenopause stage the user chose, so every perimenopause profile was stored as "unknown" and read as early; the stage is now derived from the selected option. (3) The papers described a 14-day bleeding limit while the code enforced 15 days; the papers now match the code. (4) Period logging existed only through chat; direct logging now uses the same validated writer. The same releases added a FIGO reference-range pattern check, which reads logged dates only and does not feed the forecast. [Image retention](../src/lib/chat/images.ts) · [Pattern check](../src/lib/prediction/cycle-check.ts)

Release 0.16.0 ships the first change selected by that process: the late window (forecast-v2.1.0), confirmed on unseen seeds before promotion. Seven round-1 and four round-2 changes to the everyday forecast were rejected, including one that passed its rule on the development seeds but failed on unseen ones. A locked, multi-scenario synthetic evaluation now lives in [autoresearch/](../autoresearch/). Its baseline run shows forecast-v2 under-covering noisy histories (80% coverage 0.746--0.749) and over-covering regular ones (0.875--0.886), which suggests the single within-person variance setting is too wide for regular users and too narrow for messy ones. Candidate changes are pre-registered there before they run.

After R3, the outer loop pivoted from the estimator to input-time artifact capture. Study R4-1 (pre-registered, unseen seeds 201--210, four scenarios) varied the share of missed and duplicate logs that users would correct if asked. Correcting half of the missed logs lowered the base-scenario 80% interval score 17.1% and user-macro error from 5.05 to 4.36 days; the noisy, regular and literature scenarios moved 25.2%, 11.0% and 15.7%. Correcting half of the duplicates lowered the interval score 6.6--8.9%. The best round-1 estimator change moved macro error 0.036 days. The decision was a product change, not a model change: a missed-log question after a set-aside long gap, and the chat's same-period question extended to the dashboard. The gain scales with the simulator's artifact rates (5% missed, 2% duplicate in the base scenario), which are assumptions. [R4-1 record](../autoresearch/research-log.md)
